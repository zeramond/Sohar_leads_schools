...
import { Building2, CheckCircle2, ExternalLink, Loader2, Phone, Search, Upload, Users, MessageCircle } from 'lucide-react'
...
export function LeadsDashboard({ initialStats }: Props) {
  ...
  const [search, setSearch] = useState(''); const [loading, setLoading] = useState(true); const [importOpen, setImportOpen] = useState(false); const [contacting, setContacting] = useState<number | null>(null)
  ...
  return <div className="min-h-screen hero-gradient"><header className="sticky top-0 z-20 border-b bg-background/80 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6"><h1 className="text-xl font-bold">Sohar Leads <span className="text-pink-400">CRM</span></h1><Button onClick={() => setImportOpen(true)}><Upload />Import Excel</Button></div></header>
